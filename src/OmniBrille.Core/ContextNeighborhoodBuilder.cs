...
public sealed class ContextNeighborhoodBuilder
{
        ...
        string? selectedRelationshipId = null)
    {
        ...
        if (snapshot.Relationships.Count > contextualEdges.Length)
        {
            var bounded = $"Showing {contextualEdges.Length:N0} of {snapshot.Relationships.Count:N0} authoritative relationships.";
        ...
        return new ExplorerNeighborhood(
            ...
            hiddenNodes,
            warning,
            snapshot.WasTruncated || hiddenNodes > 0 || snapshot.Relationships.Count > contextualEdges.Length,
