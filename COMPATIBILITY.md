...
|---|---|---|---|---|
| 1.1.0 | not installed | n/a | Windows x64 | Public support contract: Standalone selected-root Structure, Back/Up/Root/Trail, Search, Details, safe file activation, themes, sensory preferences, optional bundled Voice, and synchronized graph/list behavior. The exact public artifact passed automated build/package and fresh hosted install/launch/relaunch/uninstall checks; an exact-commit local package passed Windows 10 22H2 fresh-install, known-WAV Voice, and public-v1.0.0-upgrade checks. Manual visual/chrome, real hardware, and assistive-technology qualification are not claimed |
| 1.1.0 | committed v2.5 RC `59be07c6cebff12072cbf18701fb16cb11801287` | v1.0 | Windows 10 22H2 x64 | Managed protocol/session implementation is unchanged from 1.0.0 and automated Connected coverage passes, but v1.1.0 was not revalidated with this host. Connected remains compatibility-dependent rather than the primary support contract |
...
- Voice does not change Explorer Protocol compatibility. Deterministic commands are local UI actions; connected voice Search calls the already-negotiated v1 Search capability. The v1.1 Windows package owns a pinned local runtime/model; missing hardware or integrity failure affects Voice only and never blocks Standalone or Connected startup.
