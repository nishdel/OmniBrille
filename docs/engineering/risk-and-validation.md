...
- Root/native-case, ordinal `ExplorerIdentity`, and case-distinct Connected `NavigationState` history tests in [`NavigationStateTests`](../../tests/OmniBrille.Tests/NavigationStateTests.cs), plus root/bounds coverage in [`FileSystemExplorerProviderTests`](../../tests/OmniBrille.Tests/FileSystemExplorerProviderTests.cs).
