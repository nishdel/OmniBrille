...
The v1.1 shell keeps session/provider/rendering ownership unchanged while giving `GraphSceneControl` the entire client area. Root/Back, Current Focus, mode selection, provider/theme/accessibility/settings, Search, zoom, Voice, live status, and secondary panels are compact floating projections over that one graph. Search is collapsed by default, expands on its trigger or `Ctrl+F`, and continues to use `ExplorerSession` as the only query/result authority. The Current Focus chip shows the focused item name and preserves the full authorized target as tooltip and automation help. First run, Structure-empty, Search-no-match, authoritative no-Context, and filter-zero states remain distinct; Search results still take precedence over details.
