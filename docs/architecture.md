...
- `RadialGraphLayout` assigns every structural child semantic depth 1 plus a separate stable `PresentationBand`; surviving node IDs retain deterministic orientation.
...
- `GraphPresentationPolicy` owns deterministic glyph/label priority, Search emphasis, and collision rejection without Avalonia dependencies; direct structural children cannot become points.
...
Focus is semantic depth 0 at the center. Every admitted structural child is semantic depth 1 and is explicitly described as a direct child of the current focus in the graph, synchronized list, details, and automation. Density is a separate `PresentationBand`: twelve children use the inner band, sixteen the middle band, and the bounded remainder the outer band. All remain recognizable outlined folder/file glyphs with a minimum 44-DIP effective target; density never creates a fake deeper hierarchy or point-star representation. Previous focus is a distinct history/orientation node, not a parent. Stable node IDs are the continuity key: surviving nodes take the nearest deterministic slot within their presentation band while their prior coordinates remain the transition origin. On focus navigation the selected child interpolates from its old position to center and the former focus recedes into history context.

LOD combines zoom, layout scale, density, presentation band, and interaction importance without changing semantic relation:

- `Point`: reserved for non-structural ambient/context presentation; direct children are never demoted to points.
...
The v1.1 shell keeps session/provider/rendering ownership unchanged while giving `GraphSceneControl` the entire borderless client area. Root, Back, Up, Trail, Current Focus, modes, provider/theme/list/settings/Sound, Search, zoom, Voice, live status, and secondary panels are angular floating projections over that graph; custom chrome retains explicit minimize/maximize/close actions and a draggable/double-clickable title strip. Search is collapsed by default, expands on its trigger or `Ctrl+F`, and continues to use `ExplorerSession` as the only query/result authority. Details is a terminal/data deck: its full semantic text is assigned atomically while only the visual projection types, canceling on replacement. First run, Structure-empty, Search-no-match, authoritative no-Context, and filter-zero states remain distinct. Search results still take precedence over Details.

Graph semantic hierarchy and presentation density are separate. `ExplorerSceneSemantics` owns current-focus/direct-child/previous-focus/contextual/both/aggregate meaning; `PresentationBand` owns only scale, radius, and label priority. `GraphSceneControl` adds deterministic analytic low-amplitude float and a bounded hover lens around immutable base coordinates—never force simulation, topology mutation, or node admission. One approximately 24-fps foreground ticker serves transition and motion; it stops for reduced motion, hidden/minimized state, or teardown. Hit and automation bounds follow the rendered coordinates. The fixed data-rain streams retain one bounded timer/cache. Reduced motion/effects, renderer-cache limits, 44-DIP targets, geometric keyboard behavior, the synchronized accessible list, and visible-node automation remain one contract. The [interaction-state contract](interaction-state-contract.md) is authoritative for action/state parity; the retained [original concepts](assets/concepts/README.md) govern composition, not authority.
