...
Graph hierarchy is presentation-only. `GraphPresentationPolicy` combines the existing focus/selection/Search/hover state with the deterministic layout density band; `GraphSceneControl` draws focus, immediate, secondary, and ambient emphasis without changing coordinates, node admission, edge admission, or filesystem meaning. Structural edge appearance is derived from both endpoints so wire orientation cannot change its visual band. The fixed data-rain streams retain one bounded timer/cache and add only a primitive theme-aware aperture. Reduced motion/effects, renderer-cache limits, hit targets, keyboard behavior, the synchronized accessible list, and visible-node automation remain part of the same contracts.
