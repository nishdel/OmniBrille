...
This document describes the current architecture: an independently packaged standalone Structure explorer and compatibility-dependent connected Structure/Context/Hybrid explorer, plus optional local push-to-talk input. The public support contract remains Windows x64 Standalone. The Windows renderer retains Avalonia's managed SkiaSharp API but consumes a pinned project-built native asset with unused DNG/RAW support excluded; the v1.1 visual shell changes presentation without changing the established graph, state, persistence, handoff, or Explorer Protocol v1 behavior.
...
Focus is depth 0 at the center. In Structure, up to eight high-detail children form the crisp focus plane at depth 1, up to 16 more become subdued outlined glyphs at depth 2, and remaining budgeted nodes become faint point context at depth 3. These are presentation-density bands, not additional filesystem depth. Previous focus is a subdued context node. Stable node IDs are the continuity key: surviving nodes take the nearest deterministic slot within their presentation band, while their prior coordinates remain the animation origin. This preserves angular orientation without allowing files or prior page members to displace structurally preferred folders. On focus navigation the selected child interpolates from its old position to center and the former focus interpolates into context.
...
The v1.1 shell keeps session/provider/rendering ownership unchanged while giving `GraphSceneControl` the entire client area. Root/Back, Current Focus, mode selection, provider/theme/accessibility/settings, Search, zoom, Voice, live status, and secondary panels are compact floating projections over that one graph. Search is collapsed by default, expands on its trigger or `Ctrl+F`, and continues to use `ExplorerSession` as the only query/result authority. The Current Focus chip shows the focused item name and preserves the full authorized target as tooltip and automation help. First run, Structure-empty, Search-no-match, authoritative no-Context, and filter-zero states remain distinct. First-run guidance yields to open secondary surfaces, and the bottom status surface yields to side panels at the supported minimum size; neither obscures focused controls. Search results still take precedence over details.

Graph hierarchy is presentation-only. `GraphPresentationPolicy` combines the existing focus/selection/Search/hover state with the deterministic layout density band; `GraphSceneControl` draws focus, immediate, secondary, and ambient emphasis without changing node admission, edge admission, or filesystem meaning. Only primary Structure edges receive ordinary broad glow; secondary and ambient edges retain thin truthful connectivity, while selection, hover, or Search promotes a receding node and its incident edge. Structural edge appearance is derived from both endpoints so wire orientation cannot change its visual band. The fixed data-rain streams retain one bounded timer/cache and add only a primitive theme-aware aperture. Reduced motion/effects, renderer-cache limits, hit targets, keyboard behavior, the synchronized accessible list, and visible-node automation remain part of the same contracts. The retained [original concept renders](assets/concepts/README.md) govern this focus-plane composition but not product features or architecture.

No always-listening mode, wake word, cloud speech requirement, conversational/LLM assistant, destructive voice action, Hybrid voice command, indexing/database implementation, cloud service, telemetry, updater, production signing certificate, or OmniSorSe source change is implemented. All contextual relationship data in Context and Hybrid is server-authored; the Hybrid scene itself is a read-only, focus-local client composition. Incremental edge updates and durable edge selection are intentionally absent because Protocol v1 has no stable relationship ID. Real microphone validation remains outstanding and is not claimed. OmniBrille project code is MIT-licensed; bundled dependencies retain their own installed terms. The owner authorized v1.0.0 to ship unsigned with explicit SmartScreen/Unknown Publisher disclosure; a later unsigned release requires its own recorded release decision.
