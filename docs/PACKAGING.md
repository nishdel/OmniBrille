...
The Windows renderer retains Avalonia's managed SkiaSharp 3.119.4 API but excludes the official Win32 native runtime asset, which contains unused Adobe DNG/RAW code. OmniBrille instead consumes the distinctly named, repository-tracked `OmniBrille.SkiaSharp.NativeAssets.Win32.NoDng` 3.119.4.1 package. Its native DLL is built from exact upstream commits with `skia_use_dng_sdk=false`; the DNG SDK and RAW-only PIEX sources are removed before dependency sync and are neither fetched nor linked. Release validation binds the package hash and one DNG-free DLL hash through restore, publish, manifest, installation, and binary-marker checks. The project-built DLL is unsigned and is not represented as an official Microsoft-signed SkiaSharp binary. See [`native-skia.md`](native-skia.md) for authoritative provenance and upgrade procedure.
