...
The v1.2.0 descendant preview acquisition belongs only to a successful Structure overview with unused scene slots. It samples actual subfolders through the active provider, checks explicit parent authority, and does not recursively expand Context/Hybrid or invent contextual edges. Hybrid continues to compose the retained snapshot's primary Structure focus and children through its existing builder; it does not independently acquire or admit the preview collection. See [ADR 0003](decisions/0003-bounded-descendant-previews.md).
