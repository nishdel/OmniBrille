...
Package `OmniBrille.SkiaSharp.NativeAssets.Win32.NoDng` 3.119.4.2 contains the earlier twice-reproduced DLL with SHA-256 `A5A4C1EECE528A5BED7C98889435BD8214BBA610F963FE80E35256A91508B5DD`. Review later found that its CodeView record retained the hosted build directory, so it is superseded and ineligible for v1.0.0 even though no PDB file was shipped. The path-independent candidate has SHA-256 `EBE9A21F29D2474129B06FFAB67B3E74474B7F6E0D0442F14B8BAC3CFF870619`; it is not eligible for package adoption until an independent hosted rebuild reproduces the exact bytes and complete proof bundle.
...
Later inspection found that `/Brepro` normalized timestamps but did not remove the absolute CodeView PDB path. Run [32432393239](https://github.com/nishdel/OmniBrille/actions/runs/32432393239) produced the first path-independent candidate and stopped at the prior-hash gate. Its retained proof shows the literal `libSkiaSharp.pdb`, no absolute PDB path, DNG disabled and absent, 924 exports matching the official binary, and `NotSigned` status. Independent byte reproduction is still required before adoption.
